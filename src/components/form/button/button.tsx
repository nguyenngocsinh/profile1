...
export default function Button({ variant, title, className, onClick, type = 'button', form }: Props) {
  ...
  return (
    <button onClick={onClick} form={form} type={type} className={`font-body ${handleBtnColor()} ${className}`}>
      {title}
    </button>
  );
}
